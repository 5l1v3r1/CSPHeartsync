var checkincovers = require('./database/checkUser/checkinconversUser'),
    ...
    getName = require('./database/checkUser/getName'),
    getAvatar = require('./database/checkUser/getAvatar'),
...
var get_help = (senderId) => {
    sendMessage.sendTextMessageWithPromise(senderId, "Gõ một từ bất kỳ để bắt đầu một cuộc trò chuyện.").then(a => {
        sendMessage.sendTextMessageWithPromise(senderId, "Gõ \"end chat\" khi đang trò chuyện để kết thúc cuộc trò chuyện đó, hoặc khi đang ở trong hàng đợi để thoát khỏi hàng đợi").then(b => {
            sendMessage.sendTextMessageWithPromise(senderId, "Gõ \"send message\" để bắt đầu chức năng gửi lời nhắn bí mật. Bạn có thể gửi một tin nhắn đến một người dùng khác nếu bạn cung cấp được cho chatbot link facebook của nguời nhận. Tuy nhiên chatbot chỉ có thể nhắn tin đến cho người dùng của page và đã sử dụng ít nhất một lần kể từ khi update").then(c => {
                sendMessage.sendTextMessageWithPromise(senderId, "Gõ \"help\" để được trợ giúp về cách sử dụng chatbot").then(d => {
                    sendMessage.sendTextMessageWithPromise(senderId, "Và luôn nhớ rằng bạn có thể thực hiện những việc trên thông qua menu chatbot. Để truy cập vào menu chatbot, bấm vào hình có 3 gạch màu xanh ở cạnh khung nhập tin nhắn.").then(e => {
                        sendMessage.sendTextMessageWithPromise(senderId, "CSP Heartsync là sản phẩm của câu lạc bộ ADaPT, được phát triển, điều hành bởi các thành viên của câu lạc bộ ADaPT. Mã nguồn của CSP Heartsync được cung cấp mở thông qua nền tảng github dưới giấy phép MIT. Mọi chi tiết xin liên hệ với chúng tôi ở fanpage CLB: https://www.facebook.com/ADAPT.CSP/")
                    })
                })
            })
        })
    ...
    reply(senderId, textInput) {
        user_existence.check(senderId).then(in_db => {
            if (in_db === false) {
                ...
                else if (textInput.toLowerCase() === 'help') {
                ...
                } else if (textInput.toLowerCase() === 'end' || textInput.toLowerCase() === 'end chat') {
